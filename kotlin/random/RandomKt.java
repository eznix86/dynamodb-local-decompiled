...
public final class RandomKt {
    ...
    public static final void checkRangeBounds(int from2, int until) {
        boolean bl;
        boolean bl2 = bl = until > from2;
        if (!bl) {
            boolean bl3 = false;
            String string = RandomKt.boundsErrorMessage(from2, until);
            throw new IllegalArgumentException(string.toString());
        }
    }

    public static final void checkRangeBounds(long from2, long until) {
        boolean bl;
        boolean bl2 = bl = until > from2;
        if (!bl) {
            boolean bl3 = false;
    ...
    public static final void checkRangeBounds(double from2, double until) {
        boolean bl;
        boolean bl2 = bl = until > from2;
        if (!bl) {
            boolean bl3 = false;
    ...
    @NotNull
    public static final String boundsErrorMessage(@NotNull Object from2, @NotNull Object until) {
        Intrinsics.checkNotNullParameter(from2, "from");
        Intrinsics.checkNotNullParameter(until, "until");
        return "Random range is empty: [" + from2 + ", " + until + ").";
    }
}
