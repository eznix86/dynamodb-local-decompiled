...
public abstract class Random {
    ...
    public int nextInt(int from2, int until) {
        int rnd;
        boolean bl;
        RandomKt.checkRangeBounds(from2, until);
        int n = until - from2;
    ...
    public long nextLong(long from2, long until) {
        long rnd;
        boolean bl;
        RandomKt.checkRangeBounds(from2, until);
        long n = until - from2;
    ...
    public double nextDouble(double from2, double until) {
        double d;
        double d2;
        RandomKt.checkRangeBounds(from2, until);
        double size = until - from2;
        if (Double.isInfinite(size) && !Double.isInfinite(d2 = from2) && !Double.isNaN(d2) && !Double.isInfinite(d2 = until) && !Double.isNaN(d2)) {
            double r1 = this.nextDouble() * (until / (double)2 - from2 / (double)2);
            d = from2 + r1 + r1;
        } else {
            d = from2 + this.nextDouble() * size;
        }
        double r = d;
    ...
    implements Serializable {
        ...
        @Override
        public int nextInt(int from2, int until) {
            return defaultRandom.nextInt(from2, until);
        }
        ...
        @Override
        public long nextLong(long from2, long until) {
            return defaultRandom.nextLong(from2, until);
        }
        ...
        @Override
        public double nextDouble(double from2, double until) {
            return defaultRandom.nextDouble(from2, until);
        }
