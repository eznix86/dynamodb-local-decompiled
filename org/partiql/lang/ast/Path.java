/*
 * Decompiled with CFR 0.153-SNAPSHOT (d6f6758-dirty).
 * 
 * Could not load the following classes:
 *  org.jetbrains.annotations.NotNull
 *  org.jetbrains.annotations.Nullable
 */
package org.partiql.lang.ast;

import java.util.Collection;
import java.util.List;
import kotlin.Metadata;
import kotlin.collections.CollectionsKt;
import kotlin.jvm.internal.Intrinsics;
import org.jetbrains.annotations.NotNull;
import org.jetbrains.annotations.Nullable;
import org.partiql.lang.ast.AstNode;
import org.partiql.lang.ast.ExprNode;
import org.partiql.lang.ast.MetaContainer;
import org.partiql.lang.ast.PathComponent;

@Metadata(mv={1, 1, 16}, bv={1, 0, 3}, k=1, d1={"\u0000>\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0010 \n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\b\f\n\u0002\u0010\u000b\n\u0000\n\u0002\u0010\u0000\n\u0000\n\u0002\u0010\b\n\u0000\n\u0002\u0010\u000e\n\u0000\b\u0086\b\u0018\u00002\u00020\u0001B#\u0012\u0006\u0010\u0002\u001a\u00020\u0001\u0012\f\u0010\u0003\u001a\b\u0012\u0004\u0012\u00020\u00050\u0004\u0012\u0006\u0010\u0006\u001a\u00020\u0007\u00a2\u0006\u0002\u0010\bJ\t\u0010\u0012\u001a\u00020\u0001H\u00c6\u0003J\u000f\u0010\u0013\u001a\b\u0012\u0004\u0012\u00020\u00050\u0004H\u00c6\u0003J\t\u0010\u0014\u001a\u00020\u0007H\u00c6\u0003J-\u0010\u0015\u001a\u00020\u00002\b\b\u0002\u0010\u0002\u001a\u00020\u00012\u000e\b\u0002\u0010\u0003\u001a\b\u0012\u0004\u0012\u00020\u00050\u00042\b\b\u0002\u0010\u0006\u001a\u00020\u0007H\u00c6\u0001J\u0013\u0010\u0016\u001a\u00020\u00172\b\u0010\u0018\u001a\u0004\u0018\u00010\u0019H\u00d6\u0003J\t\u0010\u001a\u001a\u00020\u001bH\u00d6\u0001J\t\u0010\u001c\u001a\u00020\u001dH\u00d6\u0001R\u001a\u0010\t\u001a\b\u0012\u0004\u0012\u00020\n0\u0004X\u0096\u0004\u00a2\u0006\b\n\u0000\u001a\u0004\b\u000b\u0010\fR\u0017\u0010\u0003\u001a\b\u0012\u0004\u0012\u00020\u00050\u0004\u00a2\u0006\b\n\u0000\u001a\u0004\b\r\u0010\fR\u0014\u0010\u0006\u001a\u00020\u0007X\u0096\u0004\u00a2\u0006\b\n\u0000\u001a\u0004\b\u000e\u0010\u000fR\u0011\u0010\u0002\u001a\u00020\u0001\u00a2\u0006\b\n\u0000\u001a\u0004\b\u0010\u0010\u0011\u00a8\u0006\u001e"}, d2={"Lorg/partiql/lang/ast/Path;", "Lorg/partiql/lang/ast/ExprNode;", "root", "components", "", "Lorg/partiql/lang/ast/PathComponent;", "metas", "Lorg/partiql/lang/ast/MetaContainer;", "(Lorg/partiql/lang/ast/ExprNode;Ljava/util/List;Lorg/partiql/lang/ast/MetaContainer;)V", "children", "Lorg/partiql/lang/ast/AstNode;", "getChildren", "()Ljava/util/List;", "getComponents", "getMetas", "()Lorg/partiql/lang/ast/MetaContainer;", "getRoot", "()Lorg/partiql/lang/ast/ExprNode;", "component1", "component2", "component3", "copy", "equals", "", "other", "", "hashCode", "", "toString", "", "lang"})
public final class Path
extends ExprNode {
    @NotNull
    private final List<AstNode> children;
    @NotNull
    private final ExprNode root;
    @NotNull
    private final List<PathComponent> components;
    @NotNull
    private final MetaContainer metas;

    @Override
    @NotNull
    public List<AstNode> getChildren() {
        return this.children;
    }

    @NotNull
    public final ExprNode getRoot() {
        return this.root;
    }

    @NotNull
    public final List<PathComponent> getComponents() {
        return this.components;
    }

    @Override
    @NotNull
    public MetaContainer getMetas() {
        return this.metas;
    }

    public Path(@NotNull ExprNode root, @NotNull List<? extends PathComponent> components, @NotNull MetaContainer metas) {
        Intrinsics.checkParameterIsNotNull(root, "root");
        Intrinsics.checkParameterIsNotNull(components, "components");
        Intrinsics.checkParameterIsNotNull(metas, "metas");
        super(null);
        this.root = root;
        this.components = components;
        this.metas = metas;
        this.children = CollectionsKt.plus((Collection)CollectionsKt.listOf(this.root), (Iterable)this.components);
    }

    @NotNull
    public final ExprNode component1() {
        return this.root;
    }

    @NotNull
    public final List<PathComponent> component2() {
        return this.components;
    }

    @NotNull
    public final MetaContainer component3() {
        return this.getMetas();
    }

    @NotNull
    public final Path copy(@NotNull ExprNode root, @NotNull List<? extends PathComponent> components, @NotNull MetaContainer metas) {
        Intrinsics.checkParameterIsNotNull(root, "root");
        Intrinsics.checkParameterIsNotNull(components, "components");
        Intrinsics.checkParameterIsNotNull(metas, "metas");
        return new Path(root, components, metas);
    }

    public static /* synthetic */ Path copy$default(Path path, ExprNode exprNode, List list, MetaContainer metaContainer, int n, Object object) {
        if ((n & 1) != 0) {
            exprNode = path.root;
        }
        if ((n & 2) != 0) {
            list = path.components;
        }
        if ((n & 4) != 0) {
            metaContainer = path.getMetas();
        }
        return path.copy(exprNode, list, metaContainer);
    }

    @NotNull
    public String toString() {
        return "Path(root=" + this.root + ", components=" + this.components + ", metas=" + this.getMetas() + ")";
    }

    public int hashCode() {
        ExprNode exprNode = this.root;
        List<PathComponent> list = this.components;
        MetaContainer metaContainer = this.getMetas();
        return ((exprNode != null ? exprNode.hashCode() : 0) * 31 + (list != null ? ((Object)list).hashCode() : 0)) * 31 + (metaContainer != null ? metaContainer.hashCode() : 0);
    }

    public boolean equals(@Nullable Object object) {
        block3: {
            block2: {
                if (this == object) break block2;
                if (!(object instanceof Path)) break block3;
                Path path = (Path)object;
                if (!Intrinsics.areEqual(this.root, path.root) || !Intrinsics.areEqual(this.components, path.components) || !Intrinsics.areEqual(this.getMetas(), path.getMetas())) break block3;
            }
            return true;
        }
        return false;
    }
}

